...
public class PlanetWars {
    ...
    public void IssueOrder(Planet source, Planet dest, int numShips) {
        Console.WriteLine("" + source.PlanetID() + " " + dest.PlanetID() + " " + numShips);
	    Console.Out.Flush();
        int dist = Distance(source.PlanetID(), dest.PlanetID());
        Fleet ShadowFleet = new Fleet(1, numShips, source.PlanetID(), dest.PlanetID(), dist, dist); // Neue Flotte erstellen
        fleets.Add(ShadowFleet);
        //Planet ShadowPlanet = new Planet(source.PlanetID(), source.Owner(), source.NumShips()-numShips, source.GrowthRate(), source.X(), source.Y(), source.AvailShips(), source.IsHead(), source.TargetPlanetID()); // neuen planeten erstellen
        //planets[source.PlanetID()] = ShadowPlanet;
        planets[source.PlanetID()].AddShips(-numShips);
    }
    ...
    if (double.Parse("1.1")==11) s = s.Replace(".", ",");
